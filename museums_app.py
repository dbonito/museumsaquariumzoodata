...
URL: https://museumsaquariumzoodata-nyroryua3xkh7rrv2jpxuv.streamlit.app/
...
Description:
    ...
    All charts use custom colors, titles, and labels for a nice look.
"""
...
# Matplotlib understands these color names directly
...
def get_top_n(df, column, n=11, ascending=False):
    ...
    # Sort data in descending or ascending order
    # Find top largest or smallest values of a column
    sorted_df = df.dropna(subset=[column]).sort_values(column, ascending=ascending)
    return sorted_df.head(n)


# Function that returns more than one value
# We return mean, median and count
...
def load_data(filepath):
    ...
    # Drop any rows missing latitude or longitude, they cant be plotted on map
    ...
    # Add a new region column by mapping the numeric code using our dictionary
...
# Multi-select: user picks one or more museum types
all_types = sorted(df["Museum Type"].unique())
selected_types = st.sidebar.multiselect("🏷️ Select Museum Type(s)", options=all_types, default=all_types)

# Select: single dropdown to pick one state or all
...
# Slider: user sets a minimum and maximum revenue range
...
# Filter by one condition, keep only selected museum types
filtered = df[df["Museum Type"].isin(selected_types)]

# Filter by two conditions with AND, state AND revenue range
...
# Call revenue_stats() which returns three values at once
...
with (tab1):
    ...
    # Prepare map data: drop missing coordinates, add a revenue string
    map_df = filtered.dropna(subset=["Latitude", "Longitude"]).copy()

    # List comprehension: build Revenue_display column in one line
    ...
    # Build one sub dataframe per museum type
    sub_df_list = []
    for t in type_list:
        sub_df = map_df[map_df["Museum Type"] == t]
        sub_df_list.append(sub_df)

    # Build one ScatterplotLayer per museum type
    layer_list = []
    ...
    # ViewState: starting position and zoom
...
with tab2:
    ...
    # Horizontal bar chart, count of institutions per museum type
    with chart_col1:
        ...
        # Use the simple named color list, one color per bar
    ...
    # Pie chart: share of institutions by region
    with chart_col2:
    ...
    # Bar chart: average revenue by museum type
    with chart_col3:
        ...
        # Sort ascending so longest bar appears at the top
        avg_rev = (
    ...
    # Scatter plot: revenue vs income
    with chart_col4:
        ...
        # Cap at 97th percentile so extreme outliers don't mess up the chart
...
with tab3:
    ...
    # First call of get_top_n uses default which is ascending=FALSE
    with col_top:
    ...
    # Second call of get_top_n uses ascending=True
    with col_bot:
    ...
    st.write(
        "Pennsylvania, California, New York, and Illinois consistently lead in total reported revenue, "
        "driven by large metropolitan science museums and art institutions."
    )

    # Iterate through rows with iterrows() to build a total revenue per state
